...
use crate::backend::homeassistant::websocket::HomeAssistantWebSocket;
use crate::config::{AppConfig, HomeAssistantServer};
...
pub enum TemplateError {
    #[error("No config found for home assistant server {0:?}")]
    NotFound(String),
}

pub struct HomeAssistantServiceTemplate {
    state: AppState,
}

impl HomeAssistantServiceTemplate {
    #[must_use]
...
impl ServiceTemplate for HomeAssistantServiceTemplate {
    fn generate(&self, name: String) -> Result<BoxDynService, SvcError> {
        let config = self.state.config();
        let Some(server) = config.homeassistant.as_ref().and_then(|h| h.servers.get(&name)) else {
            return Err(SvcError::generation(TemplateError::NotFound(name)));
        };
        let svc = HomeAssistantBackend::new(name, server.clone(), config, self.state.res.clone())
...
impl HomeAssistantBackend {
    ...
    pub async fn event_loop(
        &mut self,
        _chan: &mut Receiver<Arc<BackendRequest>>,
        mut _socket: HomeAssistantWebSocket,
    ) -> ApiResult<()> {
        // loop {
        //     select! {
...
impl Service for HomeAssistantBackend {
    ...
    async fn run(&mut self) -> ApiResult<()> {
        ...
        let ha_socket = HomeAssistantWebSocket::new(self.name.clone(), socket);
        let mut chan = self.state.lock().await.backend_event_stream();
        self.event_loop(&mut chan, ha_socket).await
    }
